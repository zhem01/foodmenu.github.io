...
function addToCart(){
    ...
    if(index===-1){
        document.getElementById(itemObj.id).classList.add
        ('toggle-heart');
...
function increaseItem(){
    ...
    var inObj= cartData.find(element=>element.name==itemToInc);
    inObj.quantity+=1;
    
    currPrice=(inObj.price*inObj.quantity - inObj.price*(inObj.quantity-1))/(inObj.quantity-1);

    inObj.price=currPrice*inObj.quantity;
    totalAmount();
    cartItems();
}
var currPrice= 0;
function decreaseItem(){
 ...
 function addAddress(){
    ...
    if (address){
        ...
        //document.getElementById('m-add-address').innerText = ' ' + address;1
    }
    else{
